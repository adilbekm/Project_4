...
SESS_GET_REQUEST_BYTYPE = endpoints.ResourceContainer(
...
# - - Task 1 (Sessions) - - - - - - - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        conf = ndb.Key(urlsafe=conf_wsk).get()
        # make sure user is authed
        if not user:
            raise endpoints.UnauthorizedException('Authorization required')    
        ...
        if not conf:
            raise endpoints.NotFoundException(
                'No conference found with key: %s' % conf_wsk)   
        user_id = getUserId(user)
        conf_id = conf.key.id()
        # make sure user is the organizer of the conference
        if not user_id == conf.organizerUserId:
            raise endpoints.UnauthorizedException('User is not conference organizer')
        # make sure session name was provided
        if not request.name:
        ...
        # convert time from string to a Time object
        if data['startTime']:
            data['startTime'] = datetime.strptime(data['startTime'][:5], "%H:%M").time()
        # convert duration from string to integer
        data['duration'] = int(data['duration'])
        # add conf_id to dictionary converting it to string, add to outbound Message:
        data['conferenceId'] = str(conf_id)
        setattr(request, 'conferenceId', str(conf_id))
        
        # generate Profile Key based on user ID and Conference
        # ID based on Profile key get Conference key from ID
        p_key = ndb.Key(Conference, conf_id)
        ...
        # this block is part of Task 4.
        # add background task to set a featured speaker, if one exists
    ...
    @endpoints.method(SESS_GET_REQUEST_BYTYPE, SessionForms,
        path='getConferenceSessionsByType',
        http_method='GET', name='getConferenceSessionsByType')
    def getConferenceSessionsByType(self, request):
        """Return sessions of a specified type, given a conference"""
        # get Conference object from request; bail if not found
        conf = ndb.Key(urlsafe=request.websafeConferenceKey).get()
        if not conf:
            raise endpoints.NotFoundException(
    ...
    @endpoints.method(SESS_GET_REQUEST_BYSPEAKER, SessionForms,
...
# - - - Task 2 (Wishlist) - - - - - - - - - - - - - - - - - - - -
    ...
    def getSessionsInWishlist(self, request):
        """Get all sessions user has in their Wishlist"""
        prof = self._getProfileFromUser() # get user Profile
...
# - - - Task 3 (Additional queries) - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(message_types.VoidMessage, SessionForms,
        path='SpecialQuery1',
        http_method='GET', name='SpecialQuery1')
    ...
    @endpoints.method(message_types.VoidMessage, SessionForms,
        path='SpecialQuery2',
        http_method='GET', name='SpecialQuery2')
    def SpecialQuery2 (self, request):
        """Return all sessions longer than 60 minutes in duration"""
        sessions = Session.query().\
    ...
    @endpoints.method(message_types.VoidMessage, SessionForms,
        path='SpecialQuery3',
        http_method='GET', name='SpecialQuery3')
    def SpecialQuery3 (self, request):
        """Return all non-Workshop sessions starting before 7PM"""
        time7PM = datetime.strptime("19:00", "%H:%M").time()
        workshop = "Workshop"
        # Because datastore allows only 1 inequality per query,
        # build a list of all distinct session types, less "Workshop",
        # then use this list in the OR node of the query.
        qq = Session.query().fetch()
        types = []
        for q in qq:
            value = q.typeOfSession
            if value != workshop and not value in types:
                types.append(value)
        sessions = Session.query(ndb.OR(Session.typeOfSession.IN(types))).\
            filter(Session.startTime < time7PM)
        return SessionForms(
            items=[self._copySessionToForm(sess) for sess in sessions]
        )
...
# - - - Task 4 (Memcache) - - - - - - - - - - - - - - - - - - - -
    ...
    @staticmethod
    def _createFeaturedSpeakerAd(this_speaker, this_conf_wsk):
        """Create a featured speaker advertisement note,
           if speaker is a featured speaker, and add to memcache."""
        conf = ndb.Key(urlsafe=this_conf_wsk).get()
        conf_id = conf.key.id()
        ancestor_key=ndb.Key(Conference, conf_id)
        # check how many sessions in this conference the speaker is presenting
        q = Session.query(ancestor=ancestor_key).\
            filter(Session.speaker == this_speaker)
        countOfSessions = q.count()
        if countOfSessions > 1:
            # speaker has more than one sessions, so create a memcache entry
            sessions = q.fetch()
            note = ('%s is a featured speaker presenting at sessions %s' % (
                this_speaker,
                ', '.join(sess.name for sess in sessions))
                )
            memcache.set(MEMCACHE_FEATURED_SPEAKER_KEY, note)
    ...
    def getFeaturedSpeaker(self, request):
        """Return the Featured Speaker note from memcache"""
        note = memcache.get(MEMCACHE_FEATURED_SPEAKER_KEY)
        if not note:
            note = ""
        return StringMessage(data=note)
