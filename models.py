...
class Session(ndb.Model):
    ...
    name            = ndb.StringProperty(required=True)
    conferenceId    = ndb.StringProperty()  # ndb.KeyProperty(kind=Conference)
    highlights      = ndb.StringProperty()
    speaker         = ndb.StringProperty()
    duration        = ndb.IntegerProperty()  # in minutes
    typeOfSession   = ndb.StringProperty()
    date            = ndb.DateProperty()
    startTime       = ndb.TimeProperty()    # in 24hr notation
...
class SessionForm(messages.Message):
    ...
    duration        = messages.StringField(5) # in minutes
    typeOfSession   = messages.StringField(6)
    date            = messages.StringField(7)
    startTime       = messages.StringField(8) # needs to be in 24hr notation
